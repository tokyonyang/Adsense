...
from seo_utils import clean_text, score_keyword, is_blocked_keyword
...
def fetch_google_trends_rss(geo: str = "KR", limit: int = 30) -> pd.DataFrame:
    ...
    try:
        ...
        for e in feed.entries[:limit]:
            ...
            if title:
                rows.append({"keyword": title, "source": "google_trends_rss", "approx_traffic": approx, "collected_at": datetime.utcnow().isoformat()})
...
def load_seed_keywords(path: str = "data/seed_keywords.csv") -> pd.DataFrame:
    ...
    if "keyword" not in df.columns:
        return pd.DataFrame(columns=["keyword","source","approx_traffic","collected_at"])
    if "source" not in df.columns: df["source"] = "seed"
    if "approx_traffic" not in df.columns: df["approx_traffic"] = 0
...
def collect_keywords(geo: str = "KR", limit: int = 30, blocked_keywords: str = "") -> pd.DataFrame:
    ...
    df["score"] = df.apply(lambda r: score_keyword(r["keyword"], r.get("source",""), int(r.get("approx_traffic") or 0)), axis=1)
